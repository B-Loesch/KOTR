...
for name in st.session_state.start_df.index:
    if name in team1:
        ...
        st.session_state.delta_df.at[name, "Team"] = team_names[1]
    else:
        st.session_state.delta_df.at[name, "Team"] = team_names[2]

df_1 = st.session_state.delta_df[st.session_state.delta_df["Team"] == team_names[0]]
df_2 = st.session_state.delta_df[st.session_state.delta_df["Team"] == team_names[1]]
...
options = st.session_state.ehp_df["Region"].drop_duplicates().tolist()
options.sort()
with st.container(border=True):
    option = st.selectbox("Region Categories and EHP: ", (options))
    st.dataframe(st.session_state.ehp_df[st.session_state.ehp_df["Region"] == option].drop(columns = ["Region"]), use_container_width=True)

# st.set_page_config(layout = "wide")
# try creating pages for each tab rather than using tab layout!!!
